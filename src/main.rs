...
fn open_pool_test(txid: String, vout: u32, program_id: Pubkey, fee_txid: String, fee_vout: u32) -> String {
    ...
    println!("open_pool transaction test {:?}", result);
...
fn send_utxo() -> String {
    ...
    for (index, output) in sent_tx.output.iter().enumerate() {
        println!("output.script_pubkey {} address.script_pubkey() {} equals {}", output.script_pubkey, address.script_pubkey(), output.script_pubkey == address.script_pubkey());
    ...
    let sighash_type = TapSighashType::Default;
    let prevouts = vec![
        rpc.get_raw_transaction(&txid, None).unwrap().output[vout as usize].clone()
    ];
    let prevouts = Prevouts::All(&prevouts);
